...
| .pi/prompts/ and .pi/agents/ | Select the later-work workflow the project will use; adapt project-specific instructions and path references. If a starter was omitted, remove or rewrite role instructions that assume it exists (including generic “use the template” directions). Omit the kit's `/bootstrap` prompt and bootstrap role by default. If retaining them for a real need, rewrite both to serve this project, not to adopt a remote kit. |
